...
<body>
	...
	<div class="product" id="section1">
		<div class="container">
			...
			<form method="post"><input type="submit" name="order" value="⇵" style="background-color:rgba(0,0,0,0); border-style:none"/>
			<?php 
			$sql = "select * from gamelist order by `GameID`";
			if(isset($_POST['order']) || $_SESSION['order']==0){
				if($_SESSION['order']%2==0){$sql = "select * from gamelist order by `GameID`";}
				else{$sql = "select * from gamelist order by `GameID` desc";}
