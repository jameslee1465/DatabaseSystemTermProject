...
<head>
	...
	<title>啪奏遊戲網 Puzzle Game | 遊戲庫</title>
	<!-- css連結 -->
...
<body>
	...
	<div class="product" id="section1">
		<div class="container">
			<div class="row">
				<div class="col-12">
					<p class="title text-center">Game List</p>
					<hr class="title_line">
					<?php
			...
			<div class="row">
					...
					$GetUser = $row[0];
					$sql = "select * from orderlist where MemberID=".$GetUser." group by GameID";
					$result = mysqli_query( $conn,$sql);
					while ($row = mysqli_fetch_array($result)) {
						$sql1 = "select * from gamelist where GameID=".$row[1];
						$getgame = mysqli_query( $conn,$sql1);
						$detail = mysqli_fetch_array($getgame)
						?>
							<div class="col-md-4">
								<div class="card">
									<a href="game_detail.php?echoid=<?php echo $detail[0]; ?>">
										<div class="pic">
										<?php echo '<img src=\''.$detail[3].'\'>' ?>
										</div>
									</a>
									<p class="product_name text-right"><?php echo $detail[2] ?> /<br> <?php echo $detail[1] ?></p>
